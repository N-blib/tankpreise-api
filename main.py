...
# CORS: erlaubt alle Origins (damit dein Frontend zugreifen kann)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["GET"],
...
@app.get("/stations")
async def get_stations(lat: float, lng: float, rad: float = 5.0):
    """Tankstellen in der Nähe eines Koordinatenpunkts."""
    url = (
        f"{TK_BASE}/list.php"
        f"?lat={lat}&lng={lng}&rad={rad}"
        f"&sort=dist&type=all&apikey={TANKERKOENIG_KEY}"
    )
...
async def geocode(city: str = Query(...)):
    """Stadt → Koordinaten via Nominatim (läuft server-seitig, kein CORS-Problem)."""
    url = (
        "https://nominatim.openstreetmap.org/search"
        f"?q={city},Deutschland&format=json&limit=1&accept-language=de"
    )
    async with httpx.AsyncClient(
        timeout=8,
        headers={"User-Agent": "Tankpreis-App/1.0"}
    ) as client:
        r = await client.get(url)
    data = r.json()
    if not data:
...
async def get_market():
    """Rohöl (Brent) und EUR/USD via Yahoo Finance – server-seitig kein CORS."""
    ...
    async with httpx.AsyncClient(timeout=6) as client:
        ...
        try:
            r = await client.get(
                "https://query1.finance.yahoo.com/v8/finance/chart/EURUSD=X?interval=1d&range=2d"
            )
            d = r.json()
            closes = [c for c in d["chart"]["result"][0]["indicators"]["quote"][0]["close"] if c]
            if len(closes) >= 2:
                result["fx"] = {
                    ...
                    "change": round(closes[-1] - closes[-2], 4),
                }
        except Exception:
            pass
    return result
